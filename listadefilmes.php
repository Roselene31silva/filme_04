<?php
include "cabecalho.php";
include "banner.php";
?>
<div class="container">
    <h2>Todos os filmes</h2>
    <div class="row">
        <div class="col-3 mb-4">
            <div class="card" style="width: 18rem;">
                <img src="img/filme1.png" class="card-img-top">
                <div class="card-body">
                    <h5 class="card-title">Jurrassic Park</h5>
                    <p class="card-text">⭐ 10/10</p>
                    <a href="umfilme.php" class="btn btn-primary">Veja detalhes></a>
                </div>
            </div>
        </div>
        <div class="col-3 mb-4">
            <div class="card" style="width: 18rem;">
                <img src="img/filme2.png" class="card-img-top">
                <div class="card-body">
                    <h5 class="card-title">karate Kid Legends</h5>
                    <p class="card-text">⭐ 5/10</p>
                    <a href="umfilme.php" class="btn btn-primary">Veja detalhes</a>
                </div>
            </div>
        </div>
        <div class="col-3 mb-4">
            <div class="card" style="width: 18rem;">
                <img src="img/filme3.png" class="card-img-top">
                <div class="card-body">
                    <h5 class="card-title">Lilo e Stitch</h5>
                    <p class="card-text">⭐10/10
                    <a href="umfilme.php" class="btn btn-primary">Veja detalhes</a>
                </div>
            </div>
            <div class="col-3 mb-4">
                <div class="card" style="width: 18rem;">
                <img src="img/filme4.png" class="card-img-top">
                <div class="card-body">
                <h5 class="card-title">Pecadores</h5>
                <p class="card-text"> ⭐ 10/10</p>
                <a href="umfilme.php" class="btn btn-primary">Veja detalhes</a.
            </div>
            <div class="card-filme">
                <img src="img/filme1.png">
                <h3>Jurassic Park</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme2.png">
                <h3>Karate Kid Legends</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme3.png">
                <h3>Lilo e Stitch</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme4.png">
                <h3>Pecadores</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme1.png">
                <h3>Jurassic Park</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme2.png">
                <h3>Karate Kid Legends</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme3.png">
                <h3>Lilo e Stitch</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme4.png">
                <h3>Pecadores</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme1.png">
                <h3>Jurassic Park</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme2.png">
                <h3>Karate Kid Legends</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme3.png">
                <h3>Lilo e Stitch</h3>
                <span>⭐ 10/10</span>
            </div>
            <div class="card-filme">
                <img src="img/filme4.png">
                <h3>Pecadores</h3>
                <span>⭐ 10/10</span>
            </div>
        </div>
        <?php include "rodape.php"; ?>